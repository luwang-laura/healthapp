...
def get_show_scatter(scatter):
    ...
    row2 = html.Div([
        html.Div(children='The red line shows the average weight change with mother\'s age.', style={
            'textAlign': 'center',
            'color': colors['text']
        }),
    ])
...
def Test(data2, x_name, y_name, x_type, y_type, groups, normal=''):
    ...
    try:
        if x_type + y_type == 1 and groups == 0 and normal == True:
            ...
            words = '\nT-test result：' + 't=' + str(t) + ',P-value=' + str(p_two) + ',freedom degree=' + str(df) + '\n'
            alpha = 0.05
            if (p_two < alpha):
                words = '\n'+ words + 'P<α，reject the null hypothesis，，%sand%sthe average%shas significantly difference.\n' % \
                        (cate.name + str(cate_types[0]), cate.name + str(cate_types[1]), nums.name)
            else:
                words = '\n'+words + 'P>α，accept the null hupothesis，%sand%sthe average%shas no significant difference.\n' % \
                        (cate.name + str(cate_types[0]), cate.name + str(cate_types[1]), nums.name)
            ...
            words = '\nChe-test result：' + 'chisq-statistic=' + str(chi[0]) + ',P-value=' + str(chi[1]) + ',freedom degree=' + str(chi[2]) + '\n'
            alpha = 0.05
            if (chi[1] < alpha):
                words = words + '\nP<α，Reject the null hypothesis，%sand%shas significant difference.\n' % (x_name, y_name)
            else:
                words = words + '\nP>α，Accept the null hypothesis，%sand%shas no significant difference.\n' % (x_name, y_name)
            ...
            words = '\nThe result of the variance test is：' + 'F=' + str(aov_test[0]) + ',P-value=' + str(aov_test[1])
            alpha = 0.05
            if (aov_test[1] < alpha):
                words = words + '\nP<α，Reject the null hypothesis，%shas significant difference%s.\n' % (cate.name, nums.name)
            else:
                words = words + '\nP>α，Accept the null hypothesis，%swont have significant difference%s.\n' % (cate.name, nums.name)
        else:
...
def get_data1(file_name):
    ...
    smoke = data2[data2[factor_name] == 1][x_name]  # Mother smoking baby weight
    nosmoke = data2[data2[factor_name] == 2][x_name]  # Mother not smoking baby weight
