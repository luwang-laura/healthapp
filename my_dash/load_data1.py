...
def Test(data2, x_name, y_name, x_type, y_type, groups, normal=''):
    ...
    try:
        if x_type + y_type == 1 and groups == 0 and normal == True:
            ...
            words = 't检验结果为：' + 't=' + str(t) + ',P值=' + str(p_two) + ',自由度=' + str(df)
            alpha = 0.05
            if (p_two < alpha):
                words = words + '\nP<α，拒绝原假设，%s和%s的平均%s显著差异。' % \
                        (cate.name + str(cate_types[0]), cate.name + str(cate_types[1]), nums.name)
            else:
                words = words + '\nP>α，不能拒绝原假设，%s和%s的平均%s无显著差异。' % \
                        (cate.name + str(cate_types[0]), cate.name + str(cate_types[1]), nums.name)
            ...
            words = '卡方检验结果为：' + 'chisq-statistic=' + str(chi[0]) + ',P值=' + str(chi[1]) + ',自由度=' + str(chi[2])
            alpha = 0.05
            if (chi[1] < alpha):
                words = words + '\nP<α，拒绝原假设，%s和%s的显著差异。' % (x_name, y_name)
            else:
                words = words + '\nP>α，不能拒绝原假设，%s和%s的无显著差异。' % (x_name, y_name)
            ...
            words = '方差检验结果为：' + 'F=' + str(aov_test[0]) + ',P值=' + str(aov_test[1])
            alpha = 0.05
            if (aov_test[1] < alpha):
                words = words + '\nP<α，拒绝原假设，%s会显著影响%s。' % (cate.name, nums.name)
            else:
                words = words + '\nP>α，不能拒绝原假设，%s不会显著影响%s。' % (cate.name, nums.name)
        else:
            words = '错误,请重新选择'
    except:
        words = '错误,请重新选择'
