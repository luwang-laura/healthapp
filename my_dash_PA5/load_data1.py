...
def Test(data2, x_name, y_name, x_type, y_type, groups, normal=''):
    ...
    try:
        if x_type + y_type == 1 and groups == 0 and normal == True:
            ...
            words = '\nT-test: \n' + 't=' + str(round(t,2))  + ',P-value=' + str(round(p_two,2)) + ',freedom degree=' + str(round(df,2)) + '\n'
            alpha = 0.05
            if (p_two < alpha):
                words = '\n'+ words + 'P<α，the average value of %s and %s%s has significantly difference with ' % \
                        (cate.name + str(cate_types[0]), cate.name + str(cate_types[1]), nums.name) + '\n'
            else:
                words = '\n'+words + 'P>α，the average value of %s and %s%s has no significant difference with ' % \
                        (cate.name + str(cate_types[0]), cate.name + str(cate_types[1]), nums.name) + '\n'
            ...
            words = '\nChe-test: \n' + 'chisq-statistic=' + str(round(chi[0],2)) + ',P-value=' + str(round(chi[1],2)) + ',freedom degree=' + str(round(chi[2],2)) + '\n'
            alpha = 0.05
            if (chi[1] < alpha):
                words = '\n' + words + 'P<α,%s and %s has significant difference with' % (x_name, y_name) + '\n'
            else:
                words = '\n' + words + 'P>α，%s and %s has no significant difference with'  % (x_name, y_name) + '\n'
