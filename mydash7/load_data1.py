...
def Test(data2, x_name, y_name, x_type, y_type, groups, normal=''):
    ...
    try:
        if x_type + y_type == 1 and groups == 0 and normal == True:
            ...
            alpha = 0.05
            if (p_two < alpha):
                words = '\n'+ words + 'P<α，the average value of %s and %s%s has significantly difference ' % \
                        (cate.name + str(cate_types[0]), cate.name + str(cate_types[1]), nums.name) + '\n'
            else:
                words = '\n'+words + 'P>α，the average value of %s and %s%s has no significant difference ' % \
                        (cate.name + str(cate_types[0]), cate.name + str(cate_types[1]), nums.name) + '\n'
            ...
            alpha = 0.05
            if (chi[1] < alpha):
                words = '\n' + words + 'P<α,%s and %s has significant difference' % (x_name, y_name) + '\n'
            else:
                words = '\n' + words + 'P>α，%s and %s has no significant difference'  % (x_name, y_name) + '\n'
