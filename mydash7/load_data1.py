...
def Test(data2, x_name, y_name):
    ...
    try:
        if s == 1:
            ...
            #recommend = 'Suggested Approach' + test_dict[s] + '\n'
            words = 'Che-square：'+'chisq-statistic='+str(round(chi[0],2))+',P-value=' + str(round(chi[1],2))+',freedom degree='+str(round(chi[2],2))
            alpha = 0.05 
            if(chi[1] < alpha):
...
def get_data1(file_name):
    ...
    aov_test = Test(data2, 'How do you consider weight', 'Weight at birth, pounds')  # 方差检验
    print(aov_test)
    t_test = Test(data2, 'Mother smoked when pregnant', 'Weight at birth, pounds')  # T检验
    print(t_test)
    chi_test = Test(data2, 'Mother smoked when pregnant', 'Doctor confirmed overweight')  # 卡方检验
    print(chi_test)
