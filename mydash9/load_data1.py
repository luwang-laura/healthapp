...
def Test(data2, x_name, y_name, group):
    test_dict = {1:'T-test', 2:'Che-square', 3:'Variance test'}
    if group == 'Only 2':
        s = 1
    elif group == '2 or more' and len(set(data2[y_name])) < 5:
        s = 2
    else:
        s = 3
    ...
    try:
        if s == 1:
            #T-test: One variable is Categorical;
            cate = data2[x_name] ; cate_types = data2[x_name].value_counts().index
            nums = data2[y_name] ; nums0 = nums[cate==cate_types[0]] ; nums1 = nums[cate==cate_types[1]]
            t, p_two, df = st.ttest_ind(nums0, nums1)
            recommend = 'Suggest Approach：' + test_dict[s] + '\n'
            words = recommend + 'T-test result：' + 't=' + str(round(t,2)) + ',P-value=' + str(round(p_two,2)) + ',Freedom degree=' + str(round(df,2))
            alpha = 0.05
            if(p_two < alpha):
                words = words + '\nP<α，Reject Hypothesis，%s can significantly affect %s.' % (cate.name, nums.name)
            else: 
                words = words + '\nP>α，Accept Hypothesis，%s cannot significantly affect %s.' %(cate.name, nums.name)
        elif s == 2:
            #Che-square: Two varibles are Categrical;
            table = pd.crosstab(data2[x_name], data2[y_name])
            chi = chi2_contingency(table)
            recommend = 'Suggest Approach：' + test_dict[s] + '\n'
            words = recommend + 'Che-square result：'+'chisq-statistic='+str(round(chi[0],2))+',P-value=' + str(round(chi[1],2)) +\
                    ',Freedom degree='+str(round(chi[2],2))
            alpha = 0.05 
            if(chi[1] < alpha):
            ...
            #Variance Test: One variable is Categorical
            cate = data2[x_name] ; nums = data2[y_name] ; aov = []
            for i in cate.value_counts().index:
                aov.append(nums[cate==i])
            aov_test = stats.f_oneway(*aov)
            recommend = 'Suggest Approach: ' + test_dict[s] + '\n'
            words = recommend + 'Variance test result：' + 'F=' + str(round(aov_test[0],2)) + ',P-value=' + str(round(aov_test[1],2))
            alpha = 0.05
            if(aov_test[1] < alpha):
                words = words + '\nP<α，Reject Hypothesis，%s can significantly affect %s' % (cate.name, nums.name)
            else: 
...
def get_data1(file_name,xx, yy, zz):
    if xx is None:
        xx = "How do you consider weight"
    
    if yy is None:
        yy = "Weight at birth, pounds"
    ...
    aov_test = Test(data2, xx, yy, zz)  # 方差检验
    all_result = aov_test.replace("\n", "@#")
