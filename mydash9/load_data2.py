...
def Model_data(data2: pd.DataFrame):
    vars1 = ["Mother's age when born","Mother smoked when pregnant", "Receive newborn care at health facility",
    ...
    #p_fpr, p_tpr,thresh4 = roc_curve(test1_y, pred_prob3[:, 1], pos_label=1)
    ...
    # auc_score4 = roc_auc_score(test1_y, pred_prob3[:, 1])
    Roc_data = {"log": [fpr1, tpr1, auc_score1], "rft": [fpr2, tpr2, auc_score2],
                "svc": [fpr3, tpr3, auc_score3]}
    test_dict = {i: [log1[i], svc1[i], forest1[i]] for i in log1.keys()}
    test_df = pd.DataFrame.from_dict(test_dict, orient='index')
    test_df.columns = [' LogicRisk', ' SVCRisk',  ' ForestRisk']

    name_list = ['LogicRegression (area = %0.2f)' % auc_score1,
                 'RandomForest (area = %0.2f)' % auc_score2,
                 'SVC (area = %0.2f)' % auc_score3]

    x_list = [fpr1,fpr2,fpr3]
    y_list = [tpr1,tpr2,tpr3]
    z_list = [auc_score1, auc_score2, auc_score3]
...
def get_show_KMeans(kmeans):
    ...
    #label_list =["Type 1",'Type 2',"Type 3","Type 4", "Type 5"]
    for i in range(5):
        trace = go.Scatter(
    ...
    row2 = html.Div([
        html.Div(
            children="Type 4 is the majority, it represents the older the mother who smoked, the lighter the baby's weight",
            style={
                'textAlign': 'center',
                'color': colors['text']
            }),
...
def get_auc_plot(name_list, x_list, y_list, text):
    ...
    fig.add_trace(go.Scatter(x=[0,1], y=[0,1], name='base', mode='lines'))
...
def get_result_plot(data):
    ...
    color_list = px.colors.sequential.Agsunset
    ...
    row1 = html.Div([
        html.H2(
            children='Compare Medical Metrics',
            style={
                'textAlign': 'center',
                'color': colors['text']
            }
