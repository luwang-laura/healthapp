...
def ETL(data1:pd.DataFrame):
    ...
    def create_table():
        ...
        try:
           ...
           Mother smoked when pregnant integer,Receive newborn care at health facility integer,
    ...
    conn.commit()
    ##Load the newest data from the database
    conn = sqlite3.connect("database.db")
...
def get_heatmap():
    ...
    row2 = html.Div([
        html.Div(
            children="It seems that the mother’s age at childbirth, whether the mother smoked during pregnancy, and the weight of the newborn baby are extremely weakly correlated",
            style={
                'textAlign': 'center',
                'color': colors['text']
            }),
